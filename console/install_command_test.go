...
// allStacks lists every stack the installer supports, for table-driven tests.
var allStacks = []string{"vue", "react"}

func mustFileMap(t *testing.T, stack string) map[string]string {
	t.Helper()
	m, err := fileMapFor(stack)
	if err != nil {
		t.Fatalf("fileMapFor(%q) error = %v", stack, err)
	}
	return m
}
...
func TestScaffoldCreatesAllFiles(t *testing.T) {
	for _, stack := range allStacks {
		t.Run(stack, func(t *testing.T) {
			dir := chdirTemp(t)
			fileMap := mustFileMap(t, stack)

			created, skipped, err := scaffold(stack, false)
			if err != nil {
...
func TestScaffoldSkipsExistingWithoutForce(t *testing.T) {
	for _, stack := range allStacks {
		t.Run(stack, func(t *testing.T) {
			...
			if _, _, err := scaffold(stack, false); err != nil {
...
func TestScaffoldForceOverwrites(t *testing.T) {
	for _, stack := range allStacks {
		t.Run(stack, func(t *testing.T) {
			...
			created, skipped, err := scaffold(stack, true)
			if err != nil {
...
func TestScaffoldRejectsUnknownStack(t *testing.T) {
	chdirTemp(t)

	created, skipped, err := scaffold("svelte", false)
	if err == nil {
		t.Fatal("expected error for unknown stack, got nil")
	}
	if len(created) != 0 || len(skipped) != 0 {
		t.Errorf("expected nothing scaffolded, got created=%v skipped=%v", created, skipped)
	}
}
